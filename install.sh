...
install_deps() {
    ...
    sudo systemctl mask kea-dhcp4-server kea-dhcp6-server kea-ddns-server isc-dhcp-server isc-dhcp-server6 kea-dhcp-ddns-server nginx named frr || true
    sudo apt update
    # Core Packages
    sudo apt install -y isc-dhcp-server isc-dhcp-client kea openssh-client openssh-server keepalived nginx bind9 frr apparmor-utils
    # Fauxnet Packages
...
echo ""
echo "visit https://127.0.0.1/ to get started"
